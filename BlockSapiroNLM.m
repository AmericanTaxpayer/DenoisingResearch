function u = BlockSapiroNLM(u0,h)
    ...
    for ys = 0:ysteps
        for xs = 0:xsteps
            ...
            % first block always centered at (1,1)
            % x = add_len + 2 + xs*n;
            % y = add_len + 2 + ys*n;
                ...
                for i = -wsteps:wsteps
                    ...
                    for q = -alpha:alpha
                        for p = -alpha:alpha
                            ...
                            if p < alpha
                                Dx1 = Dx1 + (w(x+p+1,y+q)-w(x+p,y+q));
                                Dx2 = Dx2 + (w(x1+p+1,y1+q)-w(x1+p,y1+q));
                            end

                            if q < alpha
                                Dy1 = Dy1 + (w(x+p,y+q+1)-w(x+p,y+q));
                                Dy2 = Dy1 + (w(x1+p,y1+q+1)-w(x1+p,y1+q));
                            end
                        end
                    end
                    ...
                    Dx1 = Dx1 / (2*alpha*(2*alpha+1)); Dx2 = Dx2 / (2*alpha*(2*alpha+1));
                    Dy1 = Dy1 / (2*alpha*(2*alpha+1)); Dy2 = Dy2 / (2*alpha*(2*alpha+1));
