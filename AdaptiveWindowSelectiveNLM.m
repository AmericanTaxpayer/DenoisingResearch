function u = AdaptiveWindowSelectiveNLM(u0,h)
                    ...
                    for i = -window_radius:window_radius
                        ...
                        for q = -patch_radius:patch_radius
                            for p = -patch_radius:patch_radius
                                ...
                                %ssim compute mean
                                mean_A = mean_A + w(x+p,y+q,n);
                                mean_B = mean_B + w(x1+p,y1+q,n);
                            end
                        end

                        % ssim condition
                        mean_A = mean_A/((2*patch_radius+1)^2);
                        mean_B = mean_B/((2*patch_radius+1)^2);

                        % Compute variance
                        for q = -patch_radius:patch_radius
                            for p = -patch_radius:patch_radius
                                %ssim compute mean
                                var_A = var_A + (w(x+p,y+q,n)-mean_A)^2;
                                var_B = var_B + (w(x1+p,y1+q,n)-mean_B)^2;

                                cov = cov + (w(x+p,y+q,n)-mean_A)*(w(x1+p,y1+q,n)-mean_B);
                            end
                        end

                        var_A = var_A/((2*patch_radius+1)^2);
                        var_B = var_B/((2*patch_radius+1)^2);

                        cov = cov/((2*patch_radius+1)^2);
